...
from stereoigev.update import BasicMultiUpdateBlock
from stereoigev.extractor import MultiBasicEncoder, Feature
from stereoigev.geometry import Combined_Geo_Encoding_Volume
from stereoigev.submodule import (
    BasicConv,
    BasicConv_IN,
    Conv2x_IN,
    Conv2x,
...
class IGEVStereo(nn.Module):
    def __init__(self, args):
        ...
        self.cnet = MultiBasicEncoder(
            output_dim=[args.hidden_dims, context_dims], norm_fn="batch", downsample=args.n_downsample
        )
            ...
            with autocast(enabled=self.args.mixed_precision):
                if self.args.n_gru_layers == 3 and self.args.slow_fast_gru:  # Update low-res ConvGRU
                    net_list = self.update_block(
                        net_list, inp_list, iter16=True, iter08=False, iter04=False, update=False
                    )
                if (
                    self.args.n_gru_layers >= 2 and self.args.slow_fast_gru
                ):  # Update low-res ConvGRU and mid-res ConvGRU
                    net_list = self.update_block(
                        net_list, inp_list, iter16=self.args.n_gru_layers == 3, iter08=True, iter04=False, update=False
                    )
