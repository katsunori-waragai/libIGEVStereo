    ...
    def __init__(
        ...
        if self.concat:
            ...
            self.conv2 = BasicConv_IN(
                out_channels * 2, out_channels * mul, False, is_3d, IN, relu, kernel_size=3, stride=1, padding=1
            )
        else:
            self.conv2 = BasicConv_IN(
...
def norm_correlation(fea1, fea2):
    cost = torch.mean(
        ((fea1 / (torch.norm(fea1, 2, 1, True) + 1e-05)) * (fea2 / (torch.norm(fea2, 2, 1, True) + 1e-05))),
        dim=1,
        keepdim=True,
    )
    return cost
