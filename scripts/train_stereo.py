...
def fetch_optimizer(args, model):
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer, args.lr, args.num_steps + 100, pct_start=0.01, cycle_momentum=False, anneal_strategy="linear"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logdir", default="./checkpoints/sceneflow", help="the directory to save logs and checkpoints"
    )
    ...
    parser.add_argument(
        "--train_iters", type=int, default=22, help="number of updates to the disparity field in each forward pass."
    )
    parser.add_argument("--wdecay", type=float, default=0.00001, help="Weight decay in optimizer.")

    # Validation parameters
    parser.add_argument(
        "--valid_iters", type=int, default=32, help="number of flow-field updates during validation forward pass"
    )
    ...
    parser.add_argument(
        "--do_flip", default=False, choices=["h", "v"], help="flip the images horizontally or vertically"
    )
    parser.add_argument(
        "--spatial_scale", type=float, nargs="+", default=[-0.2, 0.4], help="re-scale the images randomly"
    )
